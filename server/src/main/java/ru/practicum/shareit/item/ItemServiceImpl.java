...
public class ItemServiceImpl implements ItemService {
    ...
    @Override
    public Collection<ItemDtoBooking> getAllItemsUser(Long userId, Integer from, Integer size) {
        final MyPageRequest pageRequest = new MyPageRequest(from, size, Sort.by("id").ascending());
        Collection<Item> items = itemDB.findAllByUserId(userId, pageRequest);
    ...
    public Collection<ItemDto> searchItems(String text, Integer from, Integer size) {
        ...
        Collection<Item> items = new ArrayList<>(itemDB.searchItem(text,
                new MyPageRequest(from, size, Sort.by("id").ascending())));
    ...
    public CommentDto addComment(Long userId, Long itemId, CommentDto commentDto) {
        ...
        commentDto.setAuthorName(user.get().getName());
        log.info("Комментарий добавлен {}", commentDto);
        return commentDto;
    }
